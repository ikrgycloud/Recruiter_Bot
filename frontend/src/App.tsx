...
type PendingReschedule = { id: string; sender: string | null; subject: string | null; body_preview: string | null; status: 'ready' | 'needs_review'; reason?: string; event_id?: string; event_summary?: string; requested_date?: string; candidate_requested_date?: boolean; availability_date?: string; start?: string; end?: string; duration_seconds?: number; timezone?: string; used_next_available_date?: boolean }
    ...
    <div className="approval-list">
      {items.length ? items.map(item => {
        ...
        return <article className="card" key={item.id}>
          ...
          {item.body_preview && <p className="mail-preview">{item.body_preview}</p>}
          {proposedStart && proposedEnd && <p>Suggested time: <b>{formatSystemTimestamp(proposedStart)}</b> to <b>{formatSystemTime(proposedEnd)}</b>{!item.candidate_requested_date ? ' (no date was provided; this is the next available slot)' : ''}{item.used_next_available_date ? ' (next available date after ' + item.requested_date + ')' : ''}</p>}
